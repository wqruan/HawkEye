...
class Program(object):
    ...
    def __init__(self, args, options=defaults, name=None):
        ...
        self._security = 40
        self.modulus = [int(item.strip()) for item in options.modulus.split(',')]
    ...
    def run_tapes(self, args):
        ...
        def aggregator(node_list):
            ...
            res = node_list[0]
            # print(self.globalbuildingblock)
            # print(res['online', 'round'])
    ...
    def finalize(self):
        ...
        def merge_res(targe, input):
            targe['onlinecb'] += input['online communication', 'bits']
            targe['onlinecr'] += int(input['online', 'round'])
            targe['offlinecb'] += input['offline communication', 'bits']
            targe['offlinecr'] += int(input['offline', 'round'])
        
        def print_single(inputv, sumv, form = 'GB'):
            divsor = 8*1024*1024*1024 if form == 'GB' else 8*1024*1024
            print('online comm size:'+ str(round(inputv['onlinecb']*1.0/divsor,2)) + form+'\t' + 'proportion:' + str(round(inputv['onlinecb']*1.0/sumv['onlinecb']*100, 2))+'%')
            print('online comm round:'+ str(inputv['onlinecr']) + '\t' + 'proportion:' + str(round(inputv['onlinecr']*1.0/sumv['onlinecr']*100,2))+'%')
            print('offline comm size:'+ str(round(inputv['offlinecb']*1.0/divsor,2)) + form+'\t' + 'proportion:' + (str(round(inputv['offlinecb']*1.0/sumv['offlinecb']*100, 2))  if sumv['offlinecr']>0 else '0.00' ) +'%')
            print('offline comm round:'+ str(inputv['offlinecr']*1.0) + '\t' + 'proportion:' + (str(round(inputv['offlinecr']*1.0/sumv['offlinecr']*100,2)) if sumv['offlinecr']>0 else '0.00') + '%')
        

        def print_cnn_inference(profiling_res, form = 'GB'):
            sum = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            conv2d = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            other_linear = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            linear = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            
            non_linear = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            for key, value in profiling_res.items():
                merge_res(sum, value)
                if 'conv2d' in key:
                    merge_res(conv2d, value)
                    merge_res(linear, value)

                if 'mul' in key or 'avg_pool2d' in key or 'mm' in key:
                    merge_res(other_linear, value)
                    merge_res(linear, value)
                    
                if 'relu' in key or 'max_pool2d' in key or 'Relu' in key:
                    merge_res(non_linear, value)
            # print(sum)
            print('Conv2d')
            print_single(conv2d, sum, form)
            print('Other linear')
            print_single(other_linear, sum, form)
            print('Linear')
            print_single(linear, sum, form)
            print('Non_linear')
            print_single(non_linear, sum, form)
            
        def print_cnn_train(profiling_res, form = 'GB'):
            sum = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            forward = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            backward = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            update = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            
            non_linear = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            for key, value in profiling_res.items():
                merge_res(sum, value)
                if 'forward' in key:
                    merge_res(forward, value)
            ...
            # print(sum)
            print('Forward')
            print_single(forward, sum, form)
            print('Backward')
            print_single(backward, sum, form)
            print('Update')
            print_single(update, sum, form)
            
            
        def print_transformer_inference(profiling_res, form = 'GB'):
            sum = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            matmul = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            gelu = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            Softmax = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            
            non_linear = {'onlinecb':0, 'onlinecr':0, 'offlinecb':0, 'offlinecr':0}
            for key, value in profiling_res.items():
                if 'singlebmm' in key or 'bmm' in key:
                    merge_res(matmul, value)
                    merge_res(sum, value)

                if  'gelu' in key:
                    merge_res(gelu, value)
                    merge_res(sum, value)
                    
                if  'softmax' in key:
                    merge_res(Softmax, value)
                    merge_res(sum, value)
                    
            # print(sum)
            print('Matmul')
            print_single(matmul, sum, form)
            print('Gelu')
            print_single(gelu, sum, form)
            print('Softmax')
            print_single(Softmax, sum, form)

                      
        if self.protocol =='CryptFlow2':
    ...
    class ReqChild(object):
        ...
        def aggregate(self, name):
            ...
            for node in self.nodes:
                value.append(node.aggregate())
            res = self.aggregator(value)
            if len(value) == 1 and ('online', 'round') in value[0].keys():
                    res['online', 'round'] = value[0]['online', 'round']
                    res['offline', 'round'] = value[0]['offline', 'round']
            try:
        ...
        def aggregate_profiling(self, name):
            ...
            for key, value in cost_store.items():
                # print(key)
                # # print(value)
                # print(self.aggregator([1]))
                res[key] = self.aggregator(value)   
                if len(value) == 1 and ('online', 'round') in value[0].keys():
                    res[key]['online', 'round'] = value[0]['online', 'round']
                    res[key]['offline', 'round'] = value[0]['offline', 'round']
                # if :
