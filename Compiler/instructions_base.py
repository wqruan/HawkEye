...
def cisc(function):
    class MergeCISC(Mergeable):
        ...
        def add_usage(self, req_node):
            ...
            for call in self.calls:
                repeat += 1
            if self.__class__.__name__ == 'Trunc' or self.__class__.__name__ == 'TruncPr':
                cost_func = program.get_cost('TruncPr')
                if cost_func == -1:
                    res = -1
                else:
                    config = program.cost_config
                    res = cost_func(config.bit_length, config._security, config.computation_security, config.f, config.n_parties, self.known_msb)
            else:
