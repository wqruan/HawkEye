...
class square(base.DataInstruction):
    ...
    def add_usage(self, req_node):
        cost_func = program.get_cost("muls")
        if cost_func == -1:
            print("The profiling results could be biased")
            print("Please config the cost of " + self.__class__.__name__ + " in cost_config.py")
            return
        size = self.get_size() * self.get_repeat()
        config = program.cost_config
        res = cost_func(config.bit_length, config._security, config.computation_security, \
            config.f, config.n_parties, program.degree, program.modulus, size) 
...
class dotprods(base.VarArgsInstruction, base.DataInstruction,
    ...
    def add_usage(self, req_node):
        ...
        for i, n in self.bases(iter(self.args)):
            ...
            tmpres = cost_func(config.bit_length, config._security, config.computation_security, config.f, config.n_parties, 1, dimension, 1, program.degree, program.modulus)
...
class conv2ds(base.DataInstruction, base.VarArgsInstruction, base.Mergeable):
    ...
    def add_usage(self, req_node):
        ...
        for i in range(0, len(self.args), 16):
            ...
            global tmp
            res = cost_func(config.bit_length, config._security, config.computation_security, config.f, config.n_parties, args[14] * args[3] * args[4] , args[7] * args[8] * args[11], 1, program.degree, program.modulus )
            tmp+=res[0]
...
class vanilla_conv2ds(base.DataInstruction, base.VarArgsInstruction, base.Mergeable):
    """ Secret 2D convolution.
    ...
    :param: number of groups (int)
    """
    code = base.opcodes['VANILLA_CONV2DS']
    arg_format = itertools.cycle(['sw','s','s','int','int','int','int','int',
    ...
    def __init__(self, *args, **kwargs):
        super(vanilla_conv2ds, self).__init__(*args, **kwargs)
        assert args[0].size == args[3] * args[4] * args[14] *args[15]
        assert args[1].size == args[5] * args[6] * args[11] * args[14]
        assert args[2].size == (args[7] * args[8] * args[11] * args[15]) //args[16]
    ...
    def add_usage(self, req_node):
        cost_func = program.get_cost("conv2d")
        if cost_func == -1:
            print("The profiling results could be biased")
            print("Please config the cost of conv2d in cost_config.py")
            return
        config = program.cost_config
        args = self.args
        online_round = 0
        ...
        for i in range(0, len(self.args), 17):
            ...
            res = cost_func(config.bit_length, config._security, config.computation_security, config.f,\
                config.n_parties, args[14], args[11], args[15], args[5], args[6],\
                    args[3], args[4], args[7], args[8], args[9], args[10], args[12],\
                        args[13], args[16], program.degree, program.modulus)
        ...
        req_node.increment(('online', 'round'), online_round)
        req_node.increment(('offline', 'round'), offline_round)
        super(vanilla_conv2ds, self).add_usage(req_node)
        args = self.args
...
class trunc_pr(base.VarArgsInstruction):
    ...
    def add_usage(self, req_node):
        cost_func = program.get_cost('TruncPr')
        if cost_func == -1:
            res = -1
        else:
            config = program.cost_config
            res = cost_func(config.bit_length, config._security, config.computation_security, config.f, config.n_parties, base.get_known_msb())
        if res == -1:
            print("The profiling results could be biased")
